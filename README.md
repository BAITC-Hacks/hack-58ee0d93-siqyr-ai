...
По умолчанию `.env` включает mock STT и mock-агентов на синтетическом совещании: всё работает без моделей, а в интерфейсе и API результат помечен `source_mode=mock`. Демо без входа (`AUTH_MODE=disabled`).
