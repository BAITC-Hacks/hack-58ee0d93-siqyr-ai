...
def test_confirmed_status_cannot_hide_deadline_conflict(client):
    run_id, proposal = pending(client)
    proposal["assignments"][0].update(
        review_status="confirmed", review_reasons=[],
        deadline_candidates=["завтра", "до пятницы"],
    )
    saved = save(client, run_id, proposal, 1).json()["proposal"]
    assert "deadline_conflict" in saved["assignments"][0]["review_reasons"]
    blocked = client.post(f"/api/runs/{run_id}/approve", json={"approved": True, "expected_revision": 2})
    assert blocked.status_code == 409
    assert blocked.json()["assignments"] == [1]


def test_assignment_speaker_mapping_must_be_confirmed(client):
    run_id, proposal = pending(client)
    relevant_labels = {
        proposal["segments"][index]["speaker"]
        for assignment in proposal["assignments"] if assignment["assignee"]
        for index in assignment["source_segments"]
        if proposal["speakers"].get(proposal["segments"][index]["speaker"]) == assignment["assignee"]
    }
    assert relevant_labels
    label = next(iter(relevant_labels))
    for record in proposal["speaker_records"]:
        if record["label"] == label:
            record["mapping_status"] = "suggested"
    saved = save(client, run_id, proposal, 1)
    assert saved.status_code == 200, saved.text
    blocked = client.post(f"/api/runs/{run_id}/approve", json={"approved": True, "expected_revision": 2})
    assert blocked.status_code == 409
    assert blocked.json()["code"] == "speaker_review_required"
    assert label in blocked.json()["speakers"]
