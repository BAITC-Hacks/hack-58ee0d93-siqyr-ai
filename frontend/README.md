...
На `/meetings/live` кнопка «Новый разговор» запрашивает микрофон и создаёт серверный run через `POST /api/runs/recordings`. Во время ожидания показывается анимация. Полученный `run_id` открывается по `/meetings/live/call/{run_id}`; звук отправляется секундными WebM-чанками, после завершения вызывается `/finish`. Запись и карточка встречи сохраняются в браузере. При явном выборе «Продолжить локально» на экране входа разговор записывается только в IndexedDB без серверной обработки.

Живые реплики появляются только если браузер поддерживает локальное распознавание выбранного языка и установлен языковой пакет. Облачный вариант Web Speech API не запускается. Если локальное распознавание недоступно, запись продолжается без живого текста; после завершения расшифровку можно поправить в карточке встречи. Экран рассчитан на одного говорящего у одного устройства.
